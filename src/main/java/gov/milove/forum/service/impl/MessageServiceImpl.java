...
import gov.milove.forum.repository.jpa.ForwardedMessageRepo;
    ...
    public void forwardMessages(ForwardMessagesDto dto) {
        ...
        for (Long chatId : dto.getToChatsIdList()) {
            ...
            List<Message> messages = messagesIdList.stream()
                    .map((messageId) -> {
                        Message message = new Message(chatId, forumUserRepo.getReferenceById(
                            Util.decodeUriComponent(dto.getEncodedSenderId())));
